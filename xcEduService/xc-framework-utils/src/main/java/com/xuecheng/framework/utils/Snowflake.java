...
/**
 * java edition of Twitter <b>Snowflake</b>, a network service for generating unique ID numbers at high scale with some simple guarantees.
 *
...
//    private static Logger
	...
	/**
	 * 'time stamp' here is defined as the number of millisecond that have elapsed since the {@link #epoch} given by users on {@link Snowflake} instance initialization
	 */
	...
	/**
	 * the unique and incrementing sequence number scoped in only one period/unit (here is ONE millisecond). its value will be increased by 1 in the same specified period and then reset to 0 for next
	 * period.
	 * <p>
	...
	public String toString() {
		return "Snowflake Settings [timestampBits=" + timestampBits + ", datacenterIdBits="
			+ datacenterIdBits
			+ ", workerIdBits=" + workerIdBits + ", sequenceBits=" + sequenceBits + ", epoch="
			+ epoch
	...
	public long[] parseId(long id) {
		...
		arr[2] = (id & diode(unusedBits + timestampBits + datacenterIdBits, workerIdBits))
			>> workerIdShift;
		arr[3] = (id & diode(unusedBits + timestampBits + datacenterIdBits + workerIdBits,
			sequenceBits));
		return arr;
	}
